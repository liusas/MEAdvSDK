...
#pragma mark - 新增方法
- (void)managerAdWillVisible:(UIView *)ad {
    if ([self.delegate respondsToSelector:@selector(adViewWillVisible:)]) {
        [self.delegate adViewWillVisible:self];
    }
}

- (void)managerAdDidClick:(UIView *)ad {
    if ([self.delegate respondsToSelector:@selector(adViewDidClick:)]) {
        [self.delegate adViewDidClick:self];
    }
}

- (void)adViewWillClose:(UIView *)ad {
    if ([self.delegate respondsToSelector:@selector(adViewWillClose:)]) {
        [self.delegate adViewWillClose:self];
    }
}
