...
+ (void)uploadImmediatlyWithLogs:(RLMResults *)logs postTimes:(NSInteger)postCount finished:(void (^)(void))finished {
    ...
    for (int i = 0; i < postCount+1;i++ ) {
        ...
        if (arrayDeveloperUrl.count) {
            ...
            dispatch_group_enter(group);
            //发起请求
            [MEAdLogModel uploadLogsToServerWithArray:arrayDeveloperUrl URL:[MEConfigManager sharedInstance].developerUrl Finished:^(BOOL success, NSError * _Nonnull error) {
                //标记离开
                dispatch_group_leave(group);
                if (success) {
                    //上传成功，删除日志
