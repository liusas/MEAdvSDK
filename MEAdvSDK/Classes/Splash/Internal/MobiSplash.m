 ...
 *  跳过
 */
- (void)splashAdDidClickSkipForManager:(MobiSplashAdManager *)splashAd {
    id<MobiSplashDelegate> delegate = [self.delegateTable objectForKey:splashAd.posid];
    if ([delegate respondsToSelector:@selector(splashAdDidClickSkip:)]) {
        [delegate splashAdDidClickSkip:self];
    }
}
