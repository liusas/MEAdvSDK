...
//        MPLogEvent([MPLogEvent error:error message:nil]);
    ...
    [self.rewardedVideoCustomEvent requestRewardedVideoWithCustomEventInfo:@{@"adunit":configuration.adUnitId} adMarkup:nil];
}
...
#warning 新增回调
- (void)rewardedVideoAdViewRenderFailForCustomEvent:(id<MobiRewardedVideoCustomEvent>)customEvent error:(NSError *_Nullable)error {
    if (self.delegate && [self.delegate respondsToSelector:@selector(rewardedVideoAdViewRenderFailForAdapter:error:)]) {
        [self.delegate rewardedVideoAdViewRenderFailForAdapter:self error:error];
    }
...
#warning 新增回调
- (void)rewardedVideoAdDidPlayFinishForCustomEvent:(id<MobiRewardedVideoCustomEvent>)customEvent didFailWithError:(NSError *_Nullable)error {
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(rewardedVideoAdDidPlayFinishForAdapter:didFailWithError:)]) {
        [self.delegate rewardedVideoAdDidPlayFinishForAdapter:self didFailWithError:error];
    }
