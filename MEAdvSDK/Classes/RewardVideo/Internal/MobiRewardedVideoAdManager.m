...
//    MPLogAdEvent(MPLogEvent.adLoadAttempt, self.posid);
    ...
    if (self.ready && !self.playedAd) {
        ...
        NSArray *configurations = [[StrategyFactory sharedInstance] getConfigurationsWithAdType:MobiAdTypeRewardedVideo sceneId:self.posid];
        if (configurations.count) {
            ...
            NSError * clearResponseError = [NSError errorWithDomain:MobiRewardedVideoAdsSDKDomain
                                                               code:MobiRewardedVideoAdErrorUnknown
...
- (void)rewardedVideoAdViewRenderFailForAdapter:(MobiRewardedVideoAdapter *)adapter error:(NSError *)error {
    self.ready = NO;
    self.playedAd = NO;

    [self.delegate rewardedVideoAdViewRenderFailForAdManager:self error:error];
}
...
- (void)rewardedVideoAdDidPlayFinishForAdapter:(MobiRewardedVideoAdapter *)adapter didFailWithError:(NSError *)error {
    [self.delegate rewardedVideoAdDidPlayFinishForAdManager:self didFailWithError:error];
}
