...
@interface MobiNativeExpressDrawView () <MPAdDestinationDisplayAgentDelegate>
...
@implementation MobiNativeExpressDrawView

- (instancetype)initWithNativeExpressDrawViewSize:(CGSize)drawViewSize delegate:(id)delegate {
    
    if (self = [super init]) {
        self.backgroundColor = [UIColor whiteColor];
        self.frame = CGRectMake(0, 0, drawViewSize.width, drawViewSize.height);
        _delegate = delegate;
        //初始化点击广告后的效果的代理类
...
- (void)refreshUIWithNativeBaseClass:(MobiAdNativeBaseClass *)nativeBase {
    ...
    if (nativeBase.style == 10201) {//信息流:单图480*360,一级样式id:102,二级样式id:10201,素材描述主图：480*360、标题、描述
        [self nativeExpressDrawViewTopTitleBottomImg:nativeBase];
    }else if (nativeBase.style == 10211) {//信息流:三图480*360,一级样式id:102,二级样式id:10211,主图：480*360 X 3、标题、描述
        [self nativeExpressDrawViewTopTitleBottomThreeImgs:nativeBase];
    }
}

//MARK: 信息流上文下图
- (void)nativeExpressDrawViewTopTitleBottomImg:(MobiAdNativeBaseClass *)nativeBase {
    ...
    if (![MobiLaunchAdCache checkImageInCacheWithURL:[NSURL URLWithString:nativeImg.url]]) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(nativeExpressAdViewRenderFailForDrawView:)]) {
            [self.delegate nativeExpressAdViewRenderFailForDrawView:self];
        }
        return;
    }
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(nativeExpressAdViewRenderSuccessForDrawView:)]) {
        [self.delegate nativeExpressAdViewRenderSuccessForDrawView:self];
    }
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(nativeExpressAdViewExposureForDrawView:)]) {
        [self.delegate nativeExpressAdViewExposureForDrawView:self];
    }
}

//MARK: 信息流上方文字，下方三张图片
- (void)nativeExpressDrawViewTopTitleBottomThreeImgs:(MobiAdNativeBaseClass *)nativeBase {
...
//MARK: 信息流仅有一张图片
- (void)nativeExpressDrawViewOnlyImg:(MobiAdNativeBaseClass *)nativeBase {
...
//MARK: 信息流左图右文
- (void)nativeExpressDrawViewLeftTitleRightImg:(MobiAdNativeBaseClass *)nativeBase {
...
{
    [self.delegate nativeExpressAdViewWillPresentScreenForDrawView:self];
}
...
{
    [self.delegate nativeExpressAdViewWillDissmissScreenForDrawView:self];
    [self.delegate nativeExpressAdViewDidDissmissScreenForDrawView:self];
}
...
-(void)clickAndPoint:(CGPoint)downPoint upPoint:(CGPoint)upPoint {
    ...
    MobiDrawViewAdReportModel *model = [MobiDrawViewAdReportModel new];
    model.clickDownPoint = downPoint;
    model.clickUpPoint = upPoint;
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(nativeExpressAdViewClickedForDrawView:reportModel:)]) {
        [self.delegate nativeExpressAdViewClickedForDrawView:self reportModel:model];
    }
...
@implementation MobiDrawViewAdReportModel
@end
